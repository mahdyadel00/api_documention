...
class OrderController extends ApiController
{
    ...
    public function store(Request $request)
    {
        ...
        // return $_product->id;
        //return ['u'=>Auth::user()->id, 'p'=>$_product->user_id];
        
        $order = Order::where(['user_id'=> Auth::user()->id, 'owner_id'=>$_product->user_id, 'status_id'=>1])->first();
        // return $order;
        ...
        foreach ($rent_products as $rent_product) {
            ...
            if ($rent_product->from_date == $rent_product->to_date) {
                $rent_product->to_date = \Carbon\Carbon::parse($rent_product->from_date)->addDays(1);
            }
        ...
        // return $rent_products[0]->quantity;
        // return $rent_products[0]->quantity;
        ...
        $data->request->add([
            ...
            'quantity'=> $rent_products[0]->quantity,
        ]);



       $add_product =  $this->add_product_to_order($data);
    ...
    public function change_status(Request $request)
    {
        ...
        if ($order && $order->status_id == 8) {
            ...
            // mo2men@date
            $to = \Carbon\Carbon::createFromFormat('Y-m-d H:s:i', $products[0]->to_date);
            $from = \Carbon\Carbon::createFromFormat('Y-m-d H:s:i', $products[0]->from_date);
            $diff_in_days = $to->diffInDays($from);
            $date =  \Carbon\Carbon::now();
            $from_date = $date->toDateTimeString();
            $to_date = $date->addDays($diff_in_days)->toDateTimeString();
            // endEdit@date
    ...
    public function confirm_to_delever(Request $request)
    {
        ...
        $extended_requests = ExtendedRequest::where('order_id', $request->order_id)->whereIN('status_id', ['14', '16'])->where('paid', 'unpaid')->get();
        ...
        foreach ($extended_requests as $key => $extended_request) {
            //  
            // $extended_request->paid = "paid";
            // $extended_request->save();
            ...
            if ($extended_request->payment_method == 'cash_on_received' || $extended_request->payment_method == 'cash_on_return') {
                ...
                if ($extended_request->application_amount + $extended_request->tax_amount != 0) {
                    ...
                    if ($order->owner->balance() >=  $extended_request->application_amount + $extended_request->tax_amount) {
                        ...
                        $order->paid_id = 1;
                        $order->remaining_application_total = 0;
                        ...
                        $order->paid_id = 2;
                        $order->remaining_application_total = $amount;
                    } else {
                        $user = User::find($order->owner_id);
                        ...
                        $order->paid_id = 3;
                        $order->remaining_application_total = $extended_request->application_amount +  $extended_request->tax_amount;
                    }
                    $order->save();
                }
    ...
    public function accept_or_refuse_extend_order(Request $request)
    {
        ...
        // // return $products;
        // foreach ($products as $pvalue) {
        //     // $pvalue->to_date = date('Y-m-d H:i:s', strtotime($pvalue->to_date . ' + ' . 500 . ' days'));
        //     return date('Y-m-d H:i:s', strtotime($pvalue->to_date . ' + ' . 60 . ' days'));
        //     // $pvalue->save();
        // }
        ...
        if ($request->payment_method == 'cash_on_received' || $request->payment_method == 'cash_on_return') {
            ...
            if ($order->application_amount + $order->tax_amount + $order->service_fees != 0) {
                ...
                if ($order->owner->balance() >  $order->application_amount + $order->tax_amount + $order->service_fees) {
                    ...
                    $order->paid_id = 3;
                    $order->remaining_application_total = $order->application_amount +  $order->tax_amount + $order->service_fees;
                }
            } else {
                $order->remaining_application_total = 0;
                $order->paid_id = 1;
    ...
    public function remove_product_from_order(Request $request)
    {
        ...
        return $this->orderRepo->remove_product_from_order($request->order_id, $request->product_id);
    }
